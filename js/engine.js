...
(function () {
  ...
  function showTab(tab) {
    if (document.body.classList.contains('playing')) goHome();
    el['home-screen'].dataset.tab = tab;
    if (el['bottom-nav']) {
      el['bottom-nav'].querySelectorAll('.bnav-btn').forEach(b =>
  ...
  function init() {
    ...
    // bottom nav (mobile)
    if (el['bottom-nav']) {
      el['bottom-nav'].querySelectorAll('.bnav-btn').forEach(btn => {
        btn.addEventListener('click', () => { FX.sound.play('click'); showTab(btn.dataset.tab); });
      });
    }

    // collapsible hero (mobile)
    if (el['hero-toggle']) {
      el['hero-toggle'].addEventListener('click', () => {
        const open = el['hero'].classList.toggle('expanded');
        el['hero-toggle'].setAttribute('aria-expanded', open ? 'true' : 'false');
        FX.sound.play('click');
      });
    }
